#!/bin/bash
#SBATCH --job-name=dpa1_pt2_nve10ps
#SBATCH -N 1
#SBATCH -n 1
#SBATCH --gpus=3090:1
#SBATCH --mem=16G
...
OUTPUT_PREFIX="${OUTPUT_PREFIX:-$REPO_ROOT/outputs/nve_run}"
...
lmp -k on g 1 -sf kk -var datafile "$DATA_PATH" -var model "$MODEL_PATH" -var output_prefix "$OUTPUT_PREFIX" -log "${OUTPUT_PREFIX}.log" -in inputs/in.nve.lammps
